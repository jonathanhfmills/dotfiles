...

{
  virtualisation.oci-containers.containers.vllm = {
    ...
    # CUDA-compiled vLLM extensions won't load, so --enforce-eager uses
    # pure PyTorch ops via ROCm backend. Performance hit but functional.
    # Qwen3.5 hybrid Mamba/attention arch also needs cudagraph_mode=none.
    entrypoint = "/bin/bash";
    cmd = [
      "-c"
      ''
